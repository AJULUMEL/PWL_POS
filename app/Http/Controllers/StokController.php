...
{
    $stok = StokModel::with(['barang', 'user', 'supplier'])->select('stok_id', 'barang_id', 'supplier_id', 'stok_jumlah', 'stok_tanggal', 'user_id');
    ...
    // if ($request->stok_jumlah == 'kurang10') {
    //     $stok->where('stok_jumlah', '<', 10);
    // } elseif ($request->stok_jumlah == 'lebih100') {
    //     $stok->where('stok_jumlah', '>', 100);
    // } elseif ($request->stok_jumlah == 'habis') {
    //     $stok->where('stok_jumlah', 0);
    // }
    ...
    return DataTables::of($stok)
            ...
            ->addColumn('aksi', function ($stok) {
                ...
                $btn = '<button onclick="modalAction(\'' . url('/barang/' . $stok->stok_id . '/show_ajax') . '\')" class="btn btn-info btn-sm">Detail</button> ';
                $btn .= '<button onclick="modalAction(\'' . url('/barang/' . $stok->stok_id . '/edit_ajax') . '\')" class="btn btn-warning btn-sm">Edit</button> ';
                $btn .= '<button onclick="modalAction(\'' . url('/barang/' . $stok->stok_id . '/delete_ajax') . '\')" class="btn btn-danger btn-sm">Hapus</button> ';
                return $btn;
            })
            ->rawColumns(['aksi'])
            ->make(true);
    ...
    public function create_ajax()
    {
        ...
        return view('stok.create_ajax')->with(['barang'=> $barang,'supplier'=> $supplier]);
    }
